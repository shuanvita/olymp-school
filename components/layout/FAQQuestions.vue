<template>
  ...
  <!-- <div class="space-y-15 mt-15 overflow-hidden max-w-(--breakpoint-layout) mx-auto"> -->
    ...
    <HeadlessTabGroup as="div" class="md:space-y-10 space-y-5">
      <HeadlessTabList
        class="flex flex-row text-[#8B8B8B] md:text-[20px] max-[550px]:text-base max-xmd:text-lg md:leading-6 not-md:text-primary-heading-md md:gap-x-5 gap-x-2 font-semibold flex-wrap gap-y-2">
        <HeadlessTab v-for="(item, i) in info.data.page.faq25ql.faq25" :key="i" v-slot="{ selected }" as="template">
          <button :class="{ 'text-primary': selected }"
            class="uppercase cursor-pointer text-nowrap hover:text-primary-dark not-ui-focus-visible:outline-none ui-focus-visible:outline-offset-4 ui-focus-visible:outline-primary">
            {{ item.faqgroupname }}
          </button>
        </HeadlessTab>
      </HeadlessTabList>
...
const response = await fetch('https://it-edu.com/mipt-school/graphql', {
  ...
  body: JSON.stringify({
    query: 'query GetFaq {page(id: "10690", idType: DATABASE_ID) {faq25ql {textpage faq25 {faqgroupname question25 {answer question}}}}}',
    variables: {},
  }),
});
