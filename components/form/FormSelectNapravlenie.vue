  ...
  <div class="space-y-1">
    <HeadlessListbox v-model="selectedPerson">
      <div class="relative mt-1">
        ...
        >
          <span class="block truncate">{{ selectedPerson ? selectedPerson.name:'Выберите направление' }}</span>
          <span
...
const people = [
  { name: 'Физика', id: 1, slug: 'physics' },
  // { name: 'Инженерная физика', id: 9 },
  { name: 'Математика', id: 2, slug: 'math' },
  { name: 'Информатика', id: 3, slug: 'informatics' },
  // { name: 'Промышленная разработка', id: 7 },
  // { name: 'Язык программирования Go', id: 8 },
  // { name: 'Machine Learning', id: 4 },
  // { name: 'Химия', id: 5 },
  { name: 'Информационная безопасность', id: 6, slug: 'cybersecurity' },
]

const route = useRoute()
const currentSlug = computed(() => route.path)
const filteredPeople = computed(() => {
  const match = people.find(p => currentSlug.value.includes(p.slug))
  return match ? [match] : people
})
const selectedPerson = ref<{ name: string; id: number; slug: string } | null>(null)

watch(
    filteredPeople,
    (list) => {
      selectedPerson.value = list[0] ?? null
    },
    { immediate: true }
)

watch(selectedPerson, (newValue) => {
  setValue(newValue ? String(newValue.id) : '')
})
